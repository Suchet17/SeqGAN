...
def prepare_generator_batch(samples, start_letter=0, device=torch.device('cpu')):
    """
...
def prepare_discriminator_data(pos_samples, neg_samples, device=torch.device('cpu')):
    """
...
def batchwise_oracle_nll(gen, oracle, num_samples, batch_size, max_seq_len, start_letter=0, device=torch.device('cpu')):
